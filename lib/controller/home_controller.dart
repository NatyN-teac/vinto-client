...
class HomeController extends GetxController {
  ...
  String tastes;
   ...
   searchProductForRecommendation()async {
    try{
      ...
      var response = await dio.post("${ApiEndPoints.BASE_URL}products/search",data: myPostdata );
      print("res: ${response.data}");
      if(response.statusCode == 200){
        var result = (response.data as List).map((e) => Product.fromJson(e)).toList();
  ...
  myPopularProducts()async {
    try{
      ...
      isLoading(true);
      dio.options.headers["Authorization"] = "Bearer $token";
      var response = await dio.get("${ApiEndPoints.BASE_URL}products", );
  ...
  nearbyproducts()async {
    try{
      ...
      isLoading(true);
      dio.options.headers["Authorization"] = "Bearer $token";
      var response = await dio.get("${ApiEndPoints.BASE_URL}products/products_around_me", );
